...
function send_ghost_data_link(ghost){
    if(hasDLLink){
        ...
        data += document.getElementById(ghost).querySelector(".ghost_evidence").innerText.trim().replaceAll("\n",", ") + (ghost == "The Mimic" ? ", *Приз.Огонёк" : "") + "\n"
        data += document.getElementById(ghost).querySelector(".ghost_behavior").innerText
        data = data.replace("Особенности","\n<b>Особенности:<b>\n")
        data = data.replace("Поведение","\n<b>Поведение:<b>\n")
        data = data.replace("Охота и Рассудок","\n<b>Охота и Рассудок:<b>\n")
        data = data.replace("Охота и Скорость","\n<b>Охота и Скорость:<b>\n")
        data = data.replace("Улика","\n<b>Улика:<b>\n")
        data = data.replace("🔊","")
...
function send_empty_data_link(){
    if(hasDLLink){
        dlws.send(JSON.stringify({"action":"GHOSTDATA","ghost":`None|<i>Нажмите на призрака, чтобы узнать, что он рассказывает и как себя ведет\n(Используйте кнопки ' [ ' и ' ] ', чтобы перебирать призраков)<i>`}))
    }
}
